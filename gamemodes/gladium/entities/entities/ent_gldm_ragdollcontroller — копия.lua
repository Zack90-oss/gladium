...
function ENT:PhysicsSimulate(phys,dt)
	...
	self:StepThink(0,ply,ragdoll)
	self:StepThink(1,ply,ragdoll)
end
...
function ENT:GetFootTrace(side)
	local ragdoll = self:GetRagdoll()
	local footr,footl = self.PhysBones.footr,self.PhysBones.footl
	local foot = footr
	if(side==1)then
...
function ENT:StepThink(side,ply,ragdoll)
	if(!self.Steps or !self.Steps[side])then return end
	...
	if(foot:GetPos():DistToSqr(step.goal)>1000)then
		self.Steps[side]=nil
	end
