...
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSets["default"][mode] = function(rag)
	...
	local aim = spinepos+ang:Forward()*weapondist+ang:Up()*-5.0+ang:Right()*(1.0+weaponright)
	aim = aim + b_spine4:GetVelocity()*engine.TickInterval()*4.5
	
	ang = ang+aimchange*3
	
	--local ang = ply:EyeAngles()+BSF_RAGDOLLCONTROL:GetAimAngleChange(ply)*6
	...
	if(ARG_Ent)then
		...
		curangs:RotateAroundAxis(curangs:Up(),180)
...
--Aim
local mode = "weapon_ar2(aim-hands)"
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSets["default"][mode] = function(rag)
	...
	local aim = spinepos+ang:Forward()*(weapondist-16)+ang:Up()*-5.0+ang:Right()*(2.0+weaponright)
	aim = aim + b_spine4:GetVelocity()*engine.TickInterval()*4.5
	local aim2 = spinepos+ang:Forward()*(weapondist-2)+ang:Up()*-7.0+ang:Right()*(-5.0+weaponright)
...
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSets["default"][mode] = function(rag)
	...
	if(time<0.5)then
		...
		curangs[2] = curangs[2]
		curangs[3] = curangs[3]
		curangs:RotateAroundAxis(claviclelang:Up(),180)
		...
		curangs:RotateAroundAxis(_curangs:Forward(),65)
