...
--//Idle
local mode = "weapon_stunstick(idle)"
...
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSets["default"][mode] = function(rag)
	...
	if(time<0.5)then
		...
		curangs[2] = curangs[2]
		curangs[3] = curangs[3]
		curangs:RotateAroundAxis(clavicleang:Up(),90)
		curangs:RotateAroundAxis(curangs:Forward(),-45)
		curangs:RotateAroundAxis(curangs:Up(),45)
...
--//arg-weapon_stunstick(corrective_atkdir)		--"arg" means needs global argument input, here ARG_Tilt
local mode = "arg-weapon_stunstick(corrective_atkdir)"
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSetsReportedTime["default"][mode] = 0.5	 -- Optional, but very useful for attack anims
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSets["default"][mode] = function(rag)
...
--//Stab
local mode = "weapon_stunstick(stab)"
...
--//Slash Right
local mode = "weapon_stunstick(slash-right)"
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSetsReportedTime["default"][mode] = 0.4	 -- Optional, but very useful for attack anims
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSets["default"][mode] = function(rag)
	local time = rag._RagAnimStart[mode]
	...
	if(time<0.15)then
		...
		curangs[2] = curangs[2]
		curangs[3] = curangs[3]
		curangs:RotateAroundAxis(clavicleang:Up(),30)
		curangs:RotateAroundAxis(curangs:Forward(),-20)
		curangs:RotateAroundAxis(curangs:Up(),135)
		BSF_RAGDOLLCONTROL:ControllPhysBone(b_upperarm,nil,curangs)
	...
	elseif(time<0.4)then
		...
		cureyeangs:RotateAroundAxis(cureyeangs:Forward(),-90)
		cureyeangs:RotateAroundAxis(cureyeangs:Up(),-90)
		cureyeangs:RotateAroundAxis(b_spine4ang:Forward(),80)
		--cureyeangs = LerpAngle(BSF_RAGDOLLCONTROL_ANIMATION:Anim_GetTimeLerp(time,passedtime,waittime,false),b_spine4:GetAngles(),cureyeangs)
		BSF_RAGDOLLCONTROL:ControllPhysBone(b_spine4,nil,cureyeangs)
		...
		curangs:RotateAroundAxis(curangs:Forward(),60)
		curangs:RotateAroundAxis(curangs:Up(),65)
		BSF_RAGDOLLCONTROL:ControllPhysBone(b_upperarm,nil,curangs)
...
--//Slash Left
local mode = "weapon_stunstick(slash-left)"
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSetsReportedTime["default"][mode] = 0.4	 -- Optional, but very useful for attack anims
BSF_RAGDOLLCONTROL_ANIMATION.AnimationSets["default"][mode] = function(rag)
	local time = rag._RagAnimStart[mode]
